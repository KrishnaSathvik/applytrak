...
// Offer received email template with celebration and next steps
function renderOfferReceivedHTML({ 
    name, 
    logoUrl, 
    ctaUrl, 
    settingsUrl, 
...
<head>
  ...
  <style>
    ...
    .header { background:linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%); text-align:center; padding:40px 24px; position:relative; }
    ...
    .celebration-card { background:linear-gradient(135deg, #8b5cf610 0%, #8b5cf605 100%); border:2px solid #8b5cf640; border-radius:20px; padding:32px; margin:24px 0; text-align:center; position:relative; }
    .celebration-card::before { content:''; position:absolute; top:-2px; left:-2px; right:-2px; bottom:-2px; background:linear-gradient(45deg, #8b5cf6, #8b5cf680, #8b5cf6); border-radius:20px; z-index:-1; opacity:0.1; }
    ...
    .celebration-subtitle { font:600 16px/1.3 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; color:#8b5cf6; margin:0 0 16px; }
    ...
    .next-steps-section { background:linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); border:1px solid #0ea5e9; border-radius:16px; padding:24px; margin:24px 0; }
    .next-steps-title { font:600 18px/1.3 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; color:#0c4a6e; margin:0 0 16px; display:flex; align-items:center; }
    .next-steps-icon { font-size:20px; margin-right:8px; }
    .next-steps-list { margin:0; padding-left:20px; }
    .next-steps-list li { font:400 16px/1.5 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; color:#0c4a6e; margin-bottom:8px; }
    ...
    .stat-number { font:700 24px/1 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; color:#8b5cf6; margin:0 0 4px; }
    ...
    .cta {
      display:inline-block; background:linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%); color:#fff !important;
    ...
    <tr><td align="center">
      <table role="presentation" class="container" width="100%" cellspacing="0" cellpadding="0" border="0">
        <tr><td class="header">
          ...
          <p class="tagline">Offer Received!</p>
        </td></tr>
        <tr><td class="content">
          <h1 class="h1">🎉 Congratulations! Offer Received!</h1>
          
          <p>Amazing news ${name}! You've received a job offer:</p>
          
          <div class="celebration-card">
            <span class="celebration-emoji">🏆</span>
            <h2 class="celebration-title">Job Offer Received!</h2>
            <div class="celebration-subtitle">Your hard work paid off!</div>
          </div>
          
          <div class="application-details">
            <h3 class="h2">Offer Details</h3>
            ...
            <div class="detail-row">
              ...
              <span class="detail-value">Offer Received</span>
            </div>
          ...
          <div class="next-steps-section">
            ...
            <ul class="next-steps-list">
              <li><strong>Review the offer carefully</strong> - Check salary, benefits, start date, and all terms</li>
              <li><strong>Ask questions</strong> - Don't hesitate to clarify anything you're unsure about</li>
              <li><strong>Negotiate if appropriate</strong> - Research market rates and consider negotiating</li>
              <li><strong>Get everything in writing</strong> - Ensure all details are documented</li>
              <li><strong>Consider your other options</strong> - If you have other applications in progress</li>
              <li><strong>Take time to decide</strong> - Don't feel pressured to respond immediately</li>
            </ul>
          </div>
          ...
          <div class="stats-grid">
            ...
            <div class="stat-card">
              <div class="stat-number">${userStats.offersReceived}</div>
          ...
          <p><strong>This is fantastic news!</strong> Getting a job offer is the ultimate validation of your job search efforts. Take time to celebrate this achievement!</p>
          
          <div class="cta-wrap">
            <a href="${ctaUrl}" class="cta">Update Application Status →</a>
          </div>
        ...
        <tr><td class="footer">
          ...
          <p style="font-size:11px; margin-top:12px; color:#9ca3af;">
            You're receiving this because an application status changed to "Offer Received".
          </p>
        </td></tr>
      </table>
    </td></tr>
...
serve(async (req) => {
  ...
  try {
    ...
    const stats = {
      ...
      offersReceived: userStats?.offersReceived || 0,
    ...
    const htmlContent = renderOfferReceivedHTML({
      name: name || 'there',
      logoUrl,
      ctaUrl,
      settingsUrl,
    ...
    const r = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "ApplyTrak <offers@applytrak.com>",
        to: email,
        subject: `🏆 Job Offer Received: ${application.company} - ApplyTrak`,
        html: htmlContent,
      }),
    });
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: 'Offer received email sent successfully',
        email: email
      }),
      { 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
