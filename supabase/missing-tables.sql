...
-- This file creates the missing tables that are referenced in the codebase

-- Create sequences first (required for table creation)
...
-- Create RLS policies for admin_emails (admin only) - if they don't exist
-- First, let's check if isadmin column exists and add it if needed
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added isadmin column to users table';
    END IF;
END $$;

-- Now create the admin policies
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'admin_emails' AND policyname = 'Admins can view admin emails') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'admin_emails' AND policyname = 'Admins can insert admin emails') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'admin_emails' AND policyname = 'Admins can update admin emails') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'admin_emails' AND policyname = 'Admins can delete admin emails') THEN
...
-- Sequences already created at the top of the script
...
-- Insert default admin email templates
...
-- Update the cleanup function to handle the new tables
